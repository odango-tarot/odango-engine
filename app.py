...
from prompt import FORTUNE_METHODS, get_system_prompt, build_user_message
...
with col2:
    st.markdown('<div class="section-label">✦ 占術</div>', unsafe_allow_html=True)
...
mode = st.radio("", options=["generate", "rewrite"],
    format_func=lambda x: "✨ 出目から生成" if x == "generate" else "✏️ 下書きをリライト",
    horizontal=True, label_visibility="collapsed", key="mode")

st.markdown('<div class="section-label">✦ 目標文字数</div>', unsafe_allow_html=True)
char_preset = st.select_slider("", options=[2000, 3000, 5000, 8000, 10000], value=8000, label_visibility="collapsed")
...
# 生成ボタン
# ─────────────────────────────────────────────
btn_label = "✨ おだんご節で執筆する" if mode == "generate" else "✏️ おだんご節にリライトする"
...
<div style="text-align:center; margin-top:48px; padding-bottom:32px;">
    <div style="font-size:11px; letter-spacing:3px; color:rgba(196,164,255,0.4);">🍡 ODANGO ENGINE v1.0 ✦ Private Use Only</div>
</div>
""", unsafe_allow_html=True)
